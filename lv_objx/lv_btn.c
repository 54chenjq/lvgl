 ...
 *  STATIC PROTOTYPES
 **********************/
#if 0 /*Use the ancestor design function*/
...
lv_obj_t * lv_btn_create(lv_obj_t * par, lv_obj_t * copy)
{
    ...
    ext->state_style[LV_BTN_STATE_REL] = lv_style_get(LV_STYLE_BTN_REL);
    ext->state_style[LV_BTN_STATE_PR] = lv_style_get(LV_STYLE_BTN_PR);
    ext->state_style[LV_BTN_STATE_TREL] = lv_style_get(LV_STYLE_BTN_TREL);
    ext->state_style[LV_BTN_STATE_TPR] = lv_style_get(LV_STYLE_BTN_TPR);
    ext->state_style[LV_BTN_STATE_INA] = lv_style_get(LV_STYLE_BTN_INA);
    ext->lpr_exec = 0;
    ext->tgl = 0;
    ...
    if(copy == NULL) {
	    ...
	    lv_obj_set_style(new_btn, ext->state_style[LV_BTN_STATE_REL]);
    }
    /*Copy 'copy'*/
    else {
...
void lv_btn_set_state(lv_obj_t * btn, lv_btn_state_t state)
{
    ...
    if(ext->state != state) {
        ext->state = state;
        lv_obj_set_style(btn, ext->state_style[state]);
    }
}
                                        ...
                                        lv_style_t * style_ina)
{
    ...
    ext->state_style[LV_BTN_STATE_REL] = style_rel;
    ext->state_style[LV_BTN_STATE_PR] = style_pr;
    ext->state_style[LV_BTN_STATE_TREL] = style_trel;
    ext->state_style[LV_BTN_STATE_TPR] = style_tpr;
    ext->state_style[LV_BTN_STATE_INA] = style_ina;

    if(lv_obj_get_style(btn) != ext->state_style[ext->state]) {
        lv_obj_set_style(btn, ext->state_style[ext->state]);
    }
